# By manish.17, contest: ITMO Academy. Дерево отрезков часть 1. 3, problem: (D) Intersecting Segments
...
for i in range(2*n):
    if b[a[i] - 1] != i:
        ans[a[i] - 1] = (b[a[i]-1] - i - 1) - 2 * min(st1.query(i, 2*n-1), st2.query(0, b[a[i]-1]-1))
        st1.update(i, 0)
